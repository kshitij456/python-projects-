...
def clear_fields():
	...
	desc.set('Food') ; payee.set('') ; amnt.set('') ; MoP.set('Cash'), date.set_date(today_date)
...
def remove_expense():
	...
	surety = mb.askyesno('Are you sure?', 'Are you sure that you want to delete the record of {values_selected[2]}')
...
def selected_expense_to_words():
	...
	message = 'Your expense can be read like:' "\nYou paid  " + str(values[4]) + "  to  " + str(values[2]) + "  for  " + str(values[3]) + "  on  " + str(values[1]) + "  via  " + str(values[5])
...
def expense_to_words_before_adding():
	...
	add_question = mb.askyesno('Read your record like: ', '{message}\n\nWould you like to see your income report?')
	...
	if add_question:
		...
		*
		FROM ExpenseTracker
		''', connector)
		df = pd.DataFrame(sql_query,columns= ['Description', 'Amount'])
		print(df)
		explode = (0.05,0.05,0.05)
		df.groupby(['Description']).sum().plot(kind='pie', y='Amount', explode=explode, autopct='%1.0f%%', shadow=True, startangle=60)
		plt.show()
	else:
		mb.showinfo('Ok', 'Please take your time to complete your records')
...
Label(data_entry_frame, text='Description:', font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=115)
dd1 = OptionMenu(data_entry_frame, desc, *['Food', 'Travel', 'Entertainment', 'Rent', 'Groceries', 'Mobile Services', 'Others'])
dd1.place(x=160, y=110)     ;     dd1.configure(width=10, font=entry_font)
...
dd1 = OptionMenu(data_entry_frame, MoP, *['Cash', 'Cheque', 'Credit Card', 'Debit Card', 'Paytm', 'Google Pay', 'Phone Pe'])
...
Button(data_entry_frame, text='Income Report', command=expense_to_words_before_adding, font=btn_font, width=30, bg=hlb_btn_bg).place(x=10,y=450)
